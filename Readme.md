...
The result dictionary uses canonical subsystem names as keys. Each value is a `Success` or `Failure` outcome.

Use `request_excise` to send an excise record and collect the same outcome type.

Do not rely on dictionary order. Prosody raises an exception if it cannot produce the complete result dictionary.
